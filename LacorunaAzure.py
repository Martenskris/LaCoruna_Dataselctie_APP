...
AZURE_BLOB_SAS_URL = st.secrets.get("AZURE_BLOB_SAS_URL", "").strip()


# =======================
# Download dataset (Azure Blob SAS URL) + fallback upload
# =======================
def download_file_from_sas_url(url: str, destination: str):
    """
    Downloadt een bestand van een Azure Blob SAS URL naar een lokale file.
    """
    req = Request(
        url,
        headers={
...
def ensure_dataset():
    ...
    - Anders: probeert via Azure Blob SAS URL te downloaden
    - Als dat faalt: toont upload fallback
    """
    ...
    # 1) Probeer via Azure Blob SAS URL indien aanwezig
    if AZURE_BLOB_SAS_URL:
        try:
            with st.status("Dataset downloaden van Azure Blob Storage…", expanded=True) as s:
                st.write("Azure Blob SAS URL gevonden in Streamlit Secrets.")
                download_file_from_sas_url(AZURE_BLOB_SAS_URL, FILE)
                s.update(label="Dataset gedownload", state="complete")
            return

        except HTTPError as e:
            st.error(f"HTTP-fout bij downloaden via SAS URL: {e.code} - {e.reason}")

        except URLError as e:
            st.error(f"Netwerkfout bij downloaden via SAS URL: {e.reason}")

        except Exception as e:
            st.error(f"Download faalde via Azure Blob SAS URL: {type(e).__name__}: {e}")

        st.warning(
            "De download via Azure Blob Storage is mislukt. "
            "Controleer of de SAS URL correct is en nog niet verlopen is, "
            "of upload het bestand handmatig."
        )

    else:
        st.warning("AZURE_BLOB_SAS_URL ontbreekt. Zet dit in Streamlit Secrets of upload het bestand handmatig.")
...
def load_filtered(columns: list[str], start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    ...
    filt = (
        (ds.field("Timestamp") >= ds.scalar(start.to_pydatetime())) &
        (ds.field("Timestamp") <= ds.scalar(end.to_pydatetime()))
    )
...
def load_preview_sample_full_range(signal: str, max_points: int = MAX_POINTS) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        "Timestamp": pd.to_datetime(reservoir_ts, errors="coerce"),
        signal: reservoir_val
    })
    try:
        df["Timestamp"] = df["Timestamp"].dt.tz_localize(None)
    except Exception:
